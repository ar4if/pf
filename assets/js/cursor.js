...
const Cursor = (() => {
  ...
  function onVisibilityChange() {
    ...
    if (!isTabVisible && cursorAnimFrame) {
      cancelAnimationFrame(cursorAnimFrame);
      cursorAnimFrame = null;
    } else if (isTabVisible && !cursorAnimFrame) {
      animate();
    }
  }
  ...
  function animate() {
    ...
    // Batch DOM writes - only update if values changed significantly
    if (outer) {
    ...
    // Fade trails - only update active trails
