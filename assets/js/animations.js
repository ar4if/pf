...
const Animations = (() => {
    ...
    window.addEventListener('scroll', () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          // Only update DOM if progress changed significantly (more than 0.5%)
  ...
  function setupSectionTransitions() {
    // Removed scrub-based section transitions to improve scroll performance
    // Individual element animations provide sufficient visual feedback
  }

  /* --- Premium Contact Section Animations --- */
  function setupContactAnimations() {
    const contactSection = document.querySelector('.contact-premium');
    ...
    // Decoration reveal
    gsap.fromTo('.contact-decoration',
      { opacity: 0, y: 20 },
      {
        opacity: 1, y: 0,
        duration: 0.8,
    ...
    // Social label reveal
    gsap.fromTo('.contact-social-label',
      { opacity: 0, y: 10 },
      {
        opacity: 1, y: 0,
        duration: 0.6,
